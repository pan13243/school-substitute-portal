...
export async function cfCreateKV(env, schoolId) {
  const r = await cfFetch(env, 'POST', '/accounts/' + env.CF_ACCOUNT_ID + '/storage/kv/namespaces',
...
export async function cfCreatePages(env, projName) {
  const r = await cfFetch(env, 'POST', '/accounts/' + env.CF_ACCOUNT_ID + '/pages/projects', {
    name: projName,
    source: {
      type: 'github',
...
export async function cfBindKV(env, projName, kvId) {
  ...
  const r = await cfFetch(env, 'PATCH', '/accounts/' + env.CF_ACCOUNT_ID + '/pages/projects/' + projName,
...
export async function cfDeploy(env, projName) {
  const r = await cfFetch(env, 'POST', '/accounts/' + env.CF_ACCOUNT_ID + '/pages/projects/' + projName + '/deployments',
...
export async function cfPollDeploy(env, projName, depId) {
  for (let i = 0; i < 80; i++) {
    await sleep(500);
    const r = await cfFetch(env, 'GET', '/accounts/' + env.CF_ACCOUNT_ID + '/pages/projects/' + projName + '/deployments/' + depId);
...
export async function cfPutSchoolMeta(env, kvId, meta) {
  const r = await cfFetch(env, 'PUT', '/accounts/' + env.CF_ACCOUNT_ID + '/storage/kv/namespaces/' + kvId + '/values/schoolMeta', meta);
